...
import { fetchCompanyIntelligence as _fetchCompanyIntelligence, type CompanyIntelligenceResult as _CompanyIntelligenceResult } from '@/lib/company-intelligence'
...
// ─── Types (re-exported from lib/company-intelligence) ───────────────────────

type EntityType = 'founder' | 'ceo' | 'cto' | 'vp' | 'investor' | 'advisor' | 'board'
type CompanyIntelligenceResult = _CompanyIntelligenceResult
...
// Implementation lives in lib/company-intelligence.ts (no Inngest/Supabase deps)
// so it can be imported by standalone scripts like scripts/dq-canary.ts.

export async function fetchCompanyIntelligence(
  companyName: string
): Promise<CompanyIntelligenceResult> {
  return _fetchCompanyIntelligence(companyName)
}
